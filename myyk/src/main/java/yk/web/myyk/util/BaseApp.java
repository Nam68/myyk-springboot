...
public class BaseApp extends KeyName {
    ...
    protected Constant getConstants() {
        return appConstants;
    }
    ...
    protected int getRandomInt(int digit) {
        ...
        for (int i = 0; i < digit; i++) {
            int decimalCount = 9;
            sb.append(String.valueOf(random.nextInt(decimalCount)));
        }
    ...
    protected String hashing(String target, String salt) {
        int times = Constant.getHashingTimes();
    ...
    protected String hashing(String target) {
        String salt = Constant.getHashingSalt();
    ...
    private SecretKey getKey() {
        return new SecretKeySpec(Constant.getEncryptKey().getBytes(), AES);
    }
